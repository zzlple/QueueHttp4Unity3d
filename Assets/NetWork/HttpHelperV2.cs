...
public delegate void ResultCallback(bool result, DownloadHandler downloadHandler);
...
public class HttpHelperV2 : MonoBehaviour
{
    ...
    /// The instance.队列对象实例
    /// </summary>
    private static HttpHelperV2[] _instance;
    /// <summary>
    /// The sand box path.
    /// </summary>
    ...
    public static void init(int queueCount)
    {
        ...
        HttpHelperV2.queueCount = queueCount;
        if (queueCount <= 0)
        {
            HttpHelperV2.queueCount = 1;
        }
        if (queueCount >= 8)
        {
            HttpHelperV2.queueCount = 8;
        }
    ...
    private static HttpHelperV2[] _singleton
    {
        get
        {
            _instance = GameObject.FindObjectsOfType<HttpHelperV2>();
            if (_instance == null || _instance.Length < queueCount || _instance.Length == 0)
            {
                ...
                temp.AddComponent(typeof(HttpHelperV2));
    ...
    private static void ReleaseResource(UnityWebRequest www, HttpMethod method)
    {
        ...
        try
        {
            ...
            if (null != www && null != www.downloadHandler)
            {


                ((DownloadHandlerAssetBundle)www.downloadHandler).assetBundle.Unload(false);
            }
            if (method != HttpMethod.CACHEORDOWNLOAD)
            {
                ...
                if (null != www && null != www.downloadHandler)
                {
                    Destroy(((DownloadHandlerTexture)www.downloadHandler).texture);
                }
        ...
        if (job != null)
        {
            for (int i = 0; i <= job.downloadRetries; i++)
            {
                ...
                switch (job.httpMethod)
                {
                    ...
                    case HttpMethod.GET:
                        www = UnityWebRequest.Get(job.url);
                        break;

                    case HttpMethod.POST:
                        www = UnityWebRequest.Post(job.url, job.wwwFrom);
                        break;
                    case HttpMethod.CACHEORDOWNLOAD:
                        www = UnityWebRequest.GetAssetBundle(job.url,(uint)job.version,0);
                        break;
                    ...
                    case HttpMethod.FILETODISK:
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                www = new UnityWebRequest(job.url);
                            }
                ...
                while (!this.www.isDone)
                {
                    ...
                    if (null != job.progress)
                    {
                        ...
                        job.progress(this.www.downloadProgress, this.www.uploadProgress);
                ...
                if (job.callback != null)
                {
                    job.callback(true, www.downloadHandler);
                    ReleaseResource(www, job.httpMethod);
                }
                ...
                if (job.diskfilecallback != null)
                {
                    ...
                    job.diskfilecallback(true, CreateFile(path, name, www.downloadHandler.data));
                    ReleaseResource(www, job.httpMethod);
